...
| **改价补全的 UX 补法**（诊断已完成，等拍） | 改价实现前 | 价格为变体级(`variant.price_cents`)。补法选项：逐变体各一个「存价格」/一行合并「保存变体」(同存价+量)/整表「保存全部」。Claude Code 倾向"一行合并保存"。拍后再实现后端 `UpdateVariantPrice`+路由+前端输入 |
| **是否支持 0 价（免费/赠品）** | 改价实现前 | 事实：后端创建校验只拒 `price_cents<0`、不拒 0（`catalog/product.go:348`），即后端已允许 0 价；"挡 0 价"可能在前端。拍后前后端+创建与改价口径一次对齐 |
