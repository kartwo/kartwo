...
| **D8：prod 明文访问下会话 cookie 的 `Secure` 标记** | **北极星计时开表前（阻塞 T2）** | 🔴 **本轮新发现，未拍板**。`admin/http.go:187` 恒 `Secure: h.secure`（=`Env=="prod"`），而北极星 T2 商家是经 `http://<IP>/admin/` **明文**建管理员并登录。按 RFC 6265bis §5.5，明文来源的 `Set-Cookie; Secure` 应被浏览器**整条忽略** → 登录不进后台。选项：A 按请求实际是否 TLS 决定 `Secure`（`r.TLS!=nil`）｜B 维持现状、明文态只能看不能登｜C 其它。**涉会话令牌明文传输的安全取舍，按纪律停下待 Derek 拍板，本轮不自行取舍**。同时决定 D7-B 逃生路要不要配醒目提示及其措辞 |
