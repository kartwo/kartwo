...
## 当前状态
- **阶段**：**M2.3（下单+防超卖）已完成，待 Derek 人工验收**（M2 最后一片）。
...
| M2 | 店面 + 购物车 + 下单（防超卖）+ SEO 基建（切 3 片） | 🟡 进行中（M2.3 待验收=M2收官） |
...
- [x] **M2.3 下单 + 库存预留防超卖**（待验收）：order/customer schema、结算表单(无JS可用)、订单确认页、原子预留防超卖、并发单测(库存5/并发20→恰好5成功)
