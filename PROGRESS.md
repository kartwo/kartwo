...
## 当前状态
- **阶段**：**M1.3（Admin 商品 CRUD API）已完成，待 Derek 人工验收**（分支 `feat/m1-data-model`）。
- **下一步**：M1.3 验收通过 → M1.4（媒体上传 + StoragePolicy；无 CGO WebP 方案届时带选项回报）。
...
| M1 | 核心数据模型 + Admin 基础 + 媒体上传 + StoragePolicy（切 5 片） | 🟡 进行中（M1.1/1.2 已验收，做 M1.3） |
...
- [x] **M1.3 Admin 商品 CRUD API**（待验收）：商品建/列/取/改/软删、变体校验、改库存、分类增列；鉴权+对象级+CSRF；单测+HTTP 测+实测
