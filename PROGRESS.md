...
> 最后更新：2026-08-21（M5.2 Shopify CSV 适配已人工验收，待 PR CI）

---

## 当前状态
- **阶段**：**M5.2 Shopify CSV 适配 ✅ 已人工验收，待 PR CI**（分支 `feat/m5-shopify-import`）。验收覆盖：两轴商品/变体、SKU、价格、库存和上架状态导入，重复文件幂等；含 `Image Src` 与第三变体轴的 Shopify 行均明确拒绝且不能确认导入。远程图片抓取/SSRF、第三变体轴与 301 仍留后续切片。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅ 已验收合主干；M5.2 Shopify CSV 适配 ✅ 已人工验收，待 PR CI） |
