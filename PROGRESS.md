...
## 当前状态
- **阶段**：**M1.5（Admin SPA）已完成，待 Derek 人工验收**（= M1 主验收）。
- **下一步**：M1.5(M1 主验收)通过 → 整个 M1 完成，合主干打 `v0.1.0`，随后进 M2。
...
| M1 | 核心数据模型 + Admin 基础 + 媒体上传 + StoragePolicy（切 5 片） | 🟡 进行中（M1.1–1.4 已验收，M1.5 待主验收） |
...
- [x] **M1.5 Admin SPA**（待验收=M1主验收）：Vue3+Vite 登录/向导、商品列表、新建(轴+变体矩阵)、编辑(基本信息/库存/传图预览)；embed 单二进制；CI 加前端构建
