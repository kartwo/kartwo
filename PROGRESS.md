...
> 最后更新：2026-07-07（M4.2.1 向导域名步骤 + 一气呵成外壳，人工验收通过，已合主干，不打 tag）
...
- **阶段**：**M4.2（向导完整化 + Admin UI 完善）进行中**。已拍板 **M4.2 先于 M4.3**、M4.2 切片。
  ...
  - **本片范围边界（明确不碰，留后续片）**：SMTP 全部并入 **M4.3**（与发信机器端到端做）；dashboard 概览 / 后台美化 / 全站提示迁 toast = **M4.2.2/M4.2.3**；slug 自动、上传进度 = 后续体验片。
...
- **下一步**：**M4.2.2**（dashboard 概览首页，或按后续排期）。之后 M4.2.3（后台美化 + 全站提示迁 toast）→ **M4.3**（SMTP 步骤 + 邮件队列 + 订单确认信）→ M4 收官（北极星计时打 `v0.4.0`）。散落待办（PayPal webhook INFO 日志、TLS 噪声日志治理、slug 自动、上传进度）按批统筹。
...
| M4 | 自动 HTTPS + 向导完整 + 30 分钟开店（北极星）**+ 承接：PayPal webhook 真实端到端验收（M3.3b-2 推迟项）** | 🟡 进行中（M4.1 ✅、M4.2.1 向导补完 ✅ 已合主干；M4.2.2/2.3 UI + M4.3 SMTP/邮件未开始；tag 待整体收官） |
...
- [ ] **M4.2 向导完整化 + Admin UI 完善**（进行中，切片）：
  ...
  - [ ] **M4.2.2 dashboard 概览首页**——未开始
  - [ ] **M4.2.3 后台整体美化 + 全站其它页面提示统一迁 toast**——未开始
