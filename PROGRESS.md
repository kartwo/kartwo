...
## 当前状态
- **阶段**：**M1.3 已验收通过**；进行 M1.4（媒体上传 + StoragePolicy）。
- **下一步**：定 WebP 编码方案后实现 M1.4。M1 各片累积于分支，全 M1 完成后合主干打 `v0.1.0`。
...
| M1 | 核心数据模型 + Admin 基础 + 媒体上传 + StoragePolicy（切 5 片） | 🟡 进行中（M1.1/1.2/1.3 已验收，做 M1.4） |
...
- [ ] M1.4 媒体上传 + StoragePolicy（无 CGO WebP 方案待拍板）— 进行中
