...
## 当前状态
- **阶段**：**M6.9 体验与可观测性硬化 ✅ 已开发，待人工验收**。新建商品时 slug 会由标题自动建议，商家手动修改后不再覆盖；PayPal 在线验签成功后仅记录事件类型，不记录签名头、webhook ID、请求正文或订单/付款引用；公网扫描导致的 `TLS handshake error` 降为 Debug，其他 HTTPS 服务错误仍为 Error。未改变 slug 唯一性校验、PayPal 验签/订单状态或 TLS/证书安全策略。
