...
> 最后更新：2026-08-23（M5.9 WebDAV 异地备份开发完成，待人工验收）

---

## 当前状态
- **阶段**：**M5.9 WebDAV 异地备份 🟡 开发完成，待人工验收**（分支 `feat/m5-remote-backup`）。可选环境变量 `KARTWO_BACKUP_WEBDAV_URL`、`KARTWO_BACKUP_WEBDAV_USERNAME`、`KARTWO_BACKUP_WEBDAV_PASSWORD` 启用异地复制；只接受不含 URL 内嵌凭证的 HTTPS 地址，不跟随重定向。本地自动 ZIP 成功并完成保留清理后，先 PUT 到远端 `.uploading` 临时对象，再发 WebDAV `MOVE` 原子发布正式文件；远端失败只记告警，不影响本地备份或店面服务。单测覆盖 TLS WebDAV 的 Basic Auth、PUT→MOVE 序列、临时/最终对象路径、拒绝 HTTP/URL 凭证/重定向及配置期拒绝；全量测试、vet、lint、govulncheck、gitleaks 均通过。
...
- **下一步**：创建 M5.9 草稿 PR；CI 全绿后配置真实 HTTPS WebDAV 目标，验证远端出现可恢复 ZIP。随后补后台配置表单和升级回滚能力。M4 的正式 `v0.4.0` 仍受既有发布 gate 约束。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅；M5.2 Shopify CSV 适配 ✅；M5.3 图片本地化与 SSRF 防护 ✅；M5.4 Shopify 旧链接 301 ✅；M5.5 本机健康诊断 ✅；M5.6 全量数据导出 ZIP ✅；M5.7 ZIP 恢复 ✅；M5.8 本地自动备份 ✅ 均已验收合主干；M5.9 WebDAV 异地备份 🟡 待人工验收） |
...
- [ ] **（M5.9 后）本地自动备份上传至 HTTPS WebDAV，远端出现同名 ZIP** —— 待人工验收。
