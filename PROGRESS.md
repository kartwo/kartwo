...
- **下一步**：确认 M5.6 验收记录提交的 CI 后合并 PR #7；恢复、定时/异地备份和升级回滚仍按独立切片继续。M4 的正式 `v0.4.0` 仍受既有发布 gate 约束。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅；M5.2 Shopify CSV 适配 ✅；M5.3 图片本地化与 SSRF 防护 ✅；M5.4 Shopify 旧链接 301 ✅；M5.5 本机健康诊断 ✅ 均已验收合主干；M5.6 全量数据导出 ZIP ✅ 已验收，待合主干） |
