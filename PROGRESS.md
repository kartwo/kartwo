...
> 最后更新：2026-09-02（M6.8 数据导出审计已人工验收）
...
| M6 | v1.1 硬化（审计/签名/i18n/法律模板/Woo导入/S3）+ 验收 | 🟡 进行中（M6.1 审计日志地基 ✅ 已验收合主干；M6.2 关键后台操作审计 ✅ 已验收合主干；M6.3 配置变更审计 ✅ 已验收合主干；M6.4 退款成功审计 ✅ 已验收；M6.5 磁盘性能边界提示 ✅ 已验收；M6.6 商品图片上传状态反馈 ✅ 已验收；M6.7 商品媒体操作审计 ✅ 已验收；M6.8 数据导出审计 ✅ 已验收；WooCommerce 导入延后） |
