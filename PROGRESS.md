...
> 最后更新：2026-08-23（M5.10 升级快照与原子迁移开发完成，待人工验收）

---

## 当前状态
- **阶段**：**M5.10 升级快照与原子迁移 🟡 开发完成，待人工验收**（分支 `feat/m5-upgrade-safety`）。仅在已有 `shop.db` 且存在待执行迁移时，启动期先生成不受日常保留策略清理的 `backups/kartwo-upgrade-<UTC>.zip` 全量快照；随后将本次全部待迁移 SQL 放在一个 SQLite 事务内执行，任一失败即整批回滚并拒绝启动。首次安装不创建空快照。`0015_upgrade_guard.sql` 为首个真实升级验证点；单测覆盖既有库快照、首次安装跳过与失败整批回滚，且全量测试、vet、lint、govulncheck、gitleaks 均通过。
...
- **下一步**：M5.10 先完成人工验收；M5.9 在具备可写 HTTPS WebDAV 目标后补验收。随后继续异地备份后台配置与升级可观测性。M4 的正式 `v0.4.0` 仍受既有发布 gate 约束。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅；M5.2 Shopify CSV 适配 ✅；M5.3 图片本地化与 SSRF 防护 ✅；M5.4 Shopify 旧链接 301 ✅；M5.5 本机健康诊断 ✅；M5.6 全量数据导出 ZIP ✅；M5.7 ZIP 恢复 ✅；M5.8 本地自动备份 ✅ 已合主干；M5.9 WebDAV 异地备份、M5.10 升级保护均待人工验收） |
...
- [ ] **（M5.10 后）已有数据目录升级前自动生成升级快照，新增迁移无损应用** —— 待人工验收。
