...
> 最后更新：2026-09-03（M6.10 英文商品内容与 SEO 辅助已开发）

---

## 当前状态
- **阶段**：**M6.10–M6.12 英文商品内容、SEO 与店面图片体验 ✅ 已人工验收**。商品的英文标题、英文描述和英文 slug 为店面正式内容；新增可保存的中文标题、中文 slug 与中文 SEO 描述辅助字段。商家仅在点击翻译按钮后才调用 DeepL，分别填入英文标题、英文 slug 或英文 SEO 描述；无自动请求、不再用拼音替代英文内容。未配置 DeepL 时，页面提示“尚未设置翻译 API”并提供前往翻译服务设置入口，不误导为重新登录；翻译设置页提供 DeepL 官方 API 申请链接。英文 SEO 描述为空时店面继续使用英文商品描述，非空时驱动页面 meta description、Open Graph 与 Product 结构化数据。每张图片可保存英文 alt，店面图片优先输出该文本，留空则回退英文商品标题。首页商品卡片使用 medium 派生图，避免放大 thumb 缩略图导致模糊。验收确认：商品英文翻译流程正常，首页商品封面清晰显示。未增加无效的 meta keywords 字段。
- **阶段**：**M6.8 数据导出审计 ✅ 已人工验收**。管理员成功生成全量 ZIP 导出包后追加 `export.create` 事件，固定对象为 `shop-data`；仅代表导出包已安全生成并开始响应，不能伪装成浏览器已完成下载。事件不记录 ZIP 路径、文件内容、订单、客户或凭证数据；后台显示为“生成数据导出包”。验收确认：生产后台审计日志出现 `admin` 的该中文事件，且既有媒体审计记录仍完整可见。
- **范围决策（2026-09-02）**：后台默认中文、店面默认英文；不在内核内置法律模板；媒体继续使用本地存储；WooCommerce 导入延期；二进制签名/公证暂不实施，保留 SHA256 与版本校验，待桌面平台成为正式支持目标后再评估。
- **阶段**：**M6.7 商品媒体操作审计 ✅ 已人工验收**。商品图片上传或删除成功后分别追加 `media.upload`、`media.delete` 事件，只记录操作人、动作和媒体公开标识；文件名、图片内容、存储路径和请求内容均不进入审计。重复删除失败不会追加事件；后台审计页显示为“上传商品图片”与“删除商品图片”。验收确认：生产后台审计日志出现 `admin` 的两类中文事件，时间顺序与删除后重新上传图片的实际操作一致。
- **阶段**：**M6.6 商品图片上传状态反馈 ✅ 已人工验收**。后台商品编辑页明确建议选择 `100KB–8MB` 的 JPG、PNG 或 WebP 图片（系统最大支持 `10MB`）；选中图片后禁用重复上传选择，并持续显示“正在上传并生成图片，请勿关闭页面。”；上传成功后沿用 toast“图片已上传”。接口为一次性请求，服务端 WebP 编码阶段没有可信百分比，故不显示伪进度条。验收确认：图片区说明正常显示；选择 WebP 后文件选择框变为不可用，页面显示持续提示；上传完成后既有 toast 与缩略图正常出现。未改变媒体格式、安全校验、磁盘护栏或存储策略。
- **阶段**：**M6.5 磁盘性能边界提示 ✅ 已人工验收**。后台诊断页将媒体目录真实容量转为“正常 / 注意 / 紧急”：可用空间低于总容量 10% 时提醒尽快导出、清理或扩容；接近既有 200 MB 上传保护线时明确提示新图片上传可能已暂停。仅新增可观测性提示，不改变本地存储、备份策略或上传准入规则。验收确认：生产服务诊断显示媒体目录总容量 `48 GB`、可用 `42 GB`、已用 `6.0 GB`，并显示“容量充足。”；数据库正常、自动备份 `12` 份、升级快照 `2` 份。
- **阶段**：**可信代理统一口径（D8-B） ✅ 已开发**。Admin 与 Storefront 均改为通过 `KARTWO_TRUSTED_PROXIES` 控制 `X-Forwarded-Proto` 采信条件：TLS 直连始终判定为 HTTPS；反代链路仅在 `RemoteAddr` 命中白名单且头为 `https` 时判定为 HTTPS。`storefront base()` 与两类 cookie 安全判定与 Admin session/CSRF 判定统一。待 Derek 人工验收与后续 PR 提交。
- **阶段**：**M6.4 退款成功审计 ✅ 已人工验收**。后台全额退款仅在支付服务成功返回后追加 `order.refund` 事件，目标仅记录订单公开标识；重复或失败退款不追加事件。后台 HTTP 层收敛为最小退款接口，回归测试覆盖“首次成功恰有一条事件、重复退款 409 且事件数不增加”；既有支付服务测试继续覆盖 Stripe 成功退款与订单 `paid → refunded` 状态变化。前端将事件显示为“退款订单”。验收确认：正式 HTTPS 站点的 Stripe 测试订单支付成功后完成全额退款；Kartwo 订单状态为“已退款”、退款记录含 Stripe refund ID，Stripe 沙盒交易状态为已退款，审计页出现 `admin` 的 `order.refund` 事件。
...
- **阶段**：**M5.9 WebDAV 异地备份 🟡 开发完成；本地备份主链路已真机验收，WebDAV 真实目标验收按需待办**（草稿 `PR #13`）。实现与 CI 已完成；后台补齐加密配置和连通/写入测试入口。无人值守启动上传仍以 `KARTWO_BACKUP_WEBDAV_*` 环境变量为准，避免冷启动解密凭证；由于尚未配置可写 HTTPS WebDAV 账号，未将协议单测替代为人工验收，也尚未合并。
- **真机验收（2026-08-26）**：已部署 `v0.4.0-rc1-59-g9cd307d-dirty` 至服务器；`/health`、管理员诊断接口、本地自动 ZIP 备份、80/443 监听均正常。实际备份目录以服务配置为准，本机为 `/data/m5restore/backups`；未执行 WebDAV 真实写入测试。
...
- **下一步**：M5.9 在具备可写 HTTPS WebDAV 目标后补验收；M4 的正式 `v0.4.0` 仍受既有发布 gate 约束。
...
| M6 | v1.1 硬化（审计/签名/i18n/法律模板/Woo导入/S3）+ 验收 | 🟡 进行中（M6.1–M6.8 均已验收；M6.9 slug 自动建议、PayPal 验签成功日志与 TLS 握手噪声治理 ✅ 已开发待验收；WooCommerce 导入延后） |
...
- [ ] **（M5.9 后）本地自动备份上传至 HTTPS WebDAV，远端出现同名 ZIP** —— 仅在 Derek 主动要求 WebDAV 测试并提供可写 HTTPS WebDAV 目标后执行。
...
- [x] **【M6.6，已人工验收】上传图片时显示"上传中"状态反馈 + "上传成功"。** 选图后禁用重复选择并显示持续状态；完成后 toast 提示成功。一次性上传接口无法诚实提供服务端 WebP 编码百分比，故不显示伪进度。
