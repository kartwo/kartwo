...
- **下一步**：等待 M5.8 验收记录提交的 CI 全绿后合并 `PR #11`；随后继续异地备份目标和升级回滚能力。M4 的正式 `v0.4.0` 仍受既有发布 gate 约束。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅；M5.2 Shopify CSV 适配 ✅；M5.3 图片本地化与 SSRF 防护 ✅；M5.4 Shopify 旧链接 301 ✅；M5.5 本机健康诊断 ✅；M5.6 全量数据导出 ZIP ✅；M5.7 ZIP 恢复 ✅ 均已验收合主干；M5.8 本地自动备份 ✅ 已人工验收，待合主干） |
