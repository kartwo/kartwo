...
- **下一步**：**第三轮 = M4 收官北极星「30 分钟开店」计时验收**（真机 VPS、新子域 `m4final.kartwo.com`、全新库 `~/kartwo-data-m4final`；T0=从 Release 页下载起表，停表=外网 HTTPS 绿锁 + 首页看到刚发布的带图商品）。**判定三档**：`>30:00` 未达标｜`15:00–30:00` 达标但无余量（条件通过，须列耗时分布与优化靶子）｜`≤15:00` 达标且有余量。**过则打 `v0.4.0`**。**前置**：D8 须先拍板（否则 T2 可能直接卡死）。散落待办（PayPal webhook INFO 日志、TLS 噪声日志治理、slug 自动、上传进度、Stripe 测试连接按钮）按批统筹。
