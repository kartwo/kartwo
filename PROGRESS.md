...
> 最后更新：2026-09-02（M6.9 体验与可观测性硬化已开发）

---

## 当前状态
- **阶段**：**M6.9 体验与可观测性硬化 ✅ 已开发，待人工验收**。新建商品时中文标题会先在浏览器本地转为无声调英文拼音 slug（如“速干衣”→`su-gan-yi`），可选配置 DeepL Developer/Growth 后会在停止输入时由服务端翻译为英文并生成 slug；未配置、失败或配额不可用时保留拼音，商家手动修改后不再覆盖。DeepL Key 加密保存、不下发浏览器；套餐页面展示额度并说明价格以 DeepL 账户结算页为准。PayPal 在线验签成功后仅记录事件类型；公网扫描导致的 `TLS handshake error` 降为 Debug，其他 HTTPS 服务错误仍为 Error。未改变 slug 唯一性校验、PayPal 验签/订单状态或 TLS/证书安全策略。
