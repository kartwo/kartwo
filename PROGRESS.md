...
## 当前状态
- **阶段**：**M3.2 已完成、待 Derek 人工验收**（支付路由 + Stripe Checkout 沙箱 + Webhook 双校验/幂等）。
- **下一步**：Derek 按 `docs/test/m3.2.md` 人工验收（填测试密钥→沙箱付一笔→订单转已付）；过则进 M3.3（PayPal 沙箱 + 退款 + 向导支付步骤）。
- **最新 git tag**：`v0.2.0`（M2）；M3 收官（M3.3 完）再打 `v0.3.0`。
...
| M3 | 支付路由 + Stripe/PayPal + 沙箱 + 退款 + 市场框架（切 3 片） | 🟡 进行中（M3.1 已验收；M3.2 待验收） |
