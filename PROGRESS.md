...
## 当前状态
- **阶段**：**M1.1 已验收通过**（Derek 人工测试 1-3 全过）；进行 M1.2（Admin 鉴权 + 向导骨架）。
- **下一步**：定 KDF 参数后实现 M1.2；M1 各片累积于 `feat/m1-data-model`，全 M1 完成后合主干打 `v0.1.0`。
...
- [ ] M1.2 Admin 鉴权 + 向导骨架（建管理员 + 设主口令）— 进行中
