...
- **M4.1 后一批小修/补全（均已 Derek 验收合主干、不打 tag）**：① CJK 竖排 bug（`214cd58`+`e45d43f`，2026-07-06）；② 商品改价缺口补全 + 0 价必填口径（`c9c9453`+`b503c2d`+`6578a96`，2026-07-07）；③ 轻量 toast 通知机制 + 视口居中，先接改价/新建提示（`b2f0d82`+`01efe0d`，2026-07-07）。
- **下一步**：本批（改价+竖排+toast）收尾完成。等 Derek 定 M4 后续先后：**M4.2**（向导完整/dashboard 概览/后台美化/**全站提示统一迁 toast**）与 **M4.3**（邮件队列）；散落待办（PayPal webhook INFO 日志、TLS 噪声日志治理、slug 自动、上传进度）按批统筹。
