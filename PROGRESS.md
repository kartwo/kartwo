...
> 最后更新：2026-08-22（M5.4 Shopify 旧链接 301 自动生成已完成开发，待人工验收）

---

## 当前状态
- **阶段**：**M5.4 Shopify 旧链接 301 自动生成 🟡 开发完成，待人工验收**（分支 `feat/m5-import-redirects`）。Shopify 导入成功时会在同一数据库事务保存 `Handle → 商品` 映射；访问旧路径 `/products/{handle}` 时，仅目标商品仍为上架状态才返回 `301 → /p/{slug}`，并保留查询参数。未知、草稿、归档和已删除商品一律返回 404，避免为不可公开商品暴露入口。迁移 `0014_shopify_redirect.sql` 可重入，新增映射、商品与导入任务同成同败。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅ 已验收合主干；M5.2 Shopify CSV 适配 ✅ 已合主干；M5.3 图片本地化与 SSRF 防护 ✅ 已验收合主干；M5.4 Shopify 旧链接 301 自动生成 🟡 待人工验收） |
