...
## 当前状态
- **阶段**：**M3.1（市场框架+向导市场+加密设置）已完成，待 Derek 人工验收**（分支 feat/m3-payments）。
- **下一步**：M2.1 验收通过 → M2.2（购物车 schema+状态机）。
- **下一步**：进 **M3**（支付路由 + Stripe/PayPal + 沙箱 + 退款）；**需先拍板"主攻市场"**。
...
| M3 | 支付路由 + Stripe/PayPal + 沙箱 + 退款 + 市场框架（切 3 片） | 🟡 进行中（M3.1 待验收） |
...
## 当前里程碑明细（M3 · 切 3 片）
- [x] **M3.1 市场框架 + 向导市场选择 + 加密设置地基**（待验收）：可扩展 Market 注册表(US 点亮/其余即将上线)、AES-GCM(KEK)加密设置、向导市场步骤(大白话文案)、店面货币随市场；单测+实测
- [ ] M3.2 支付路由 + Stripe Checkout 沙箱 + Webhook 双校验（拒伪造/幂等）
