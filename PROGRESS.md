...
## 当前状态
- **阶段**：**M2.2（购物车）已完成，待 Derek 人工验收**（分支 feat/m2-storefront）。
...
| M2 | 店面 + 购物车 + 下单（防超卖）+ SEO 基建（切 3 片） | 🟡 进行中（M2.2 待验收） |
...
- [x] **M2.2 购物车**（待验收）：匿名购物车(cookie/SameSite)、加/累加/改/删、购物车页+JSON、角标、渐进增强 cart.js、变体选择器；单测+实测（order/customer schema 按"用到才建"留 M2.3）
