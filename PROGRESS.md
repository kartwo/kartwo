...
- **下一步**：等 Derek 拍**改价补法**（变体级改价 UX + 是否支持 0 价口径）后单独实现改价——高优先级、顶"开源版必须完整能开店赚钱"底线、排在 M4.2/M4.3 之前。改价补法未定前不动改价代码。
...
- [ ] **【缺口·高优先级，顶"开源版必须完整能开店赚钱"底线】商品"创建后不能改价"——诊断完成、根因确认。** 价格是**变体级**(`variant.price_cents`，整数分)，**后端更新路径与前端编辑页均未实现改价**：后端无 `UpdateVariantPrice`/无改价路由/sqlc 无改价 UPDATE（只 create 时 INSERT 带价）；前端编辑页价格**只读展示**、无输入、`api.js` 无改价调用。非 bug、非入口问题。**待 Derek 拍改价 UX（变体级逐行/合并存价+量/批量）+ 0 价口径后单独实现；仍排在 M4.2/M4.3 之前。**
