...
## 当前状态
- **阶段**：**M1.1（数据模型与迁移）已完成，待 Derek 人工验收**（分支 `feat/m1-data-model`）。
- **下一步**：M1.1 验收通过 → 继续 M1.2（Admin 鉴权 + 向导骨架）。
...
## 当前里程碑明细（M1 · 切 5 片）
- [x] **M1.1 数据模型与迁移**（待验收）：通用双轴 option×option schema、纯 SQL 迁移、sqlc 数据层、seed-demo 装 6 变体并打印矩阵、单测
- [ ] M1.2 Admin 鉴权 + 向导骨架（建管理员 + 设主口令）
