...
> 最后更新：2026-08-22（M5.6 全量数据导出 ZIP 开发完成，待人工验收）

---

## 当前状态
- **阶段**：**M5.6 全量数据导出 ZIP 🟡 开发完成，待人工验收**（分支 `feat/m5-export`）。Admin 新增“导出”入口和受鉴权保护的 `GET /admin/api/export` 下载；按导出瞬间创建 SQLite `VACUUM INTO` 一致性快照，连同 `data/media/` 打包为 ZIP。包内含 `manifest.json`（格式版本、UTC 创建时间、应用版本）、`shop.db`、`media/`；不包含 `data/certs/` 证书缓存。HTTP 传输结束即删除临时 ZIP 和数据库快照，文件权限为 0600。单测已验证快照可重新打开、媒体内容存在、证书文件不被导出；全量测试、vet、lint、govulncheck、gitleaks 和前端构建均通过。
...
- **下一步**：创建 M5.6 草稿 PR，待 CI 全绿后进行全量导出 ZIP 的人工验收；恢复、定时/异地备份和升级回滚仍按独立切片继续。M4 的正式 `v0.4.0` 仍受既有发布 gate 约束。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅；M5.2 Shopify CSV 适配 ✅；M5.3 图片本地化与 SSRF 防护 ✅；M5.4 Shopify 旧链接 301 ✅；M5.5 本机健康诊断 ✅ 均已验收合主干；M5.6 全量数据导出 ZIP 🟡 待人工验收） |
...
- [ ] **（M5.6 后）Admin → 导出 ZIP 含 SQLite 快照和媒体文件，且不含证书缓存** —— 待人工验收。
