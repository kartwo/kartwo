...
> 最后更新：2026-08-21（M5.3 Shopify 图片本地化与 SSRF 防护已完成开发，待人工验收）

---

## 当前状态
- **阶段**：**M5.3 Shopify 图片本地化与 SSRF 防护 🟡 待人工验收**（分支 `feat/m5-shopify-images`）。M5.2 已合入 `main`；本片支持 Shopify `Image Src` 及额外图片行，以公开 HTTPS URL 下载后走既有媒体处理与本地存储。预览不联网；执行阶段限制 15 秒/10MiB、禁止重定向与非默认端口，并在 DNS 与拨号层拦截内网/本机地址。图片失败时商品、媒体记录与导入任务不提交；第三变体轴和 301 仍留后续切片。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅ 已验收合主干；M5.2 Shopify CSV 适配 ✅ 已合主干；M5.3 图片本地化与 SSRF 防护 🟡 待人工验收） |
