...
## 当前状态
- **阶段**：**M1.2（Admin 鉴权 + 向导骨架）已完成，待 Derek 人工验收**（分支 `feat/m1-data-model`）。
- **下一步**：M1.2 验收通过 → M1.3（Admin 商品 CRUD API）。M1 各片累积于分支，全 M1 完成后合主干打 `v0.1.0`。
...
| M1 | 核心数据模型 + Admin 基础 + 媒体上传 + StoragePolicy（切 5 片） | 🟡 进行中（M1.1 待验收） |
...
- [x] **M1.2 Admin 鉴权 + 向导骨架**（待验收）：argon2id 口令、主口令派生 KEK(内存金库)、初始化幂等、会话+CSRF、登录限流、向导 API；单测+HTTP 测
- [ ] M1.3 Admin 商品 CRUD API（含变体）
- [ ] M1.4 媒体上传 + StoragePolicy（无 CGO WebP 方案到时回报）
