...
> 最后更新：2026-08-21（M5.1 通用 CSV 导入已验收合入 main、CI #22 全绿；M5.2 Shopify CSV 适配开发中）

---

## 当前状态
- **阶段**：**M5.2 Shopify CSV 适配 🟡 开发中**（分支 `feat/m5-shopify-import`）。M5.1 通用 CSV 导入地基已完成人工验收并合入 `main`（合并提交 `f8b7866`，CI #22 四项全绿）。本片仅映射 Shopify 商品、两轴变体、SKU、价格、库存与状态；远程图片抓取/SSRF、第三变体轴与 301 仍留后续切片。
...
| M5 | 数据导入(含301) + 诊断页 + 备份/导出/升级 | 🟡 进行中（M5.1 通用 CSV 导入地基 ✅ 已验收合主干；M5.2 Shopify CSV 适配开发中） |
